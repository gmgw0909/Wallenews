...
//    private ImageView mCoverImage;
    ...
    public void playVideo() {
        ...
        BaseViewHolder holder = ((WaLiMultiAdapter) getAdapter()).getHolder(child);
        if (holder == null) {
            playPosition = -1;
            return;
        }
//        mCoverImage = holder.mCover;
        LinearLayout frameLayout = holder.getView(R.id.ll_img);
...
//                VideoPlayerConfig.MIN_PLAYBACK_RESUME_BUFFER, -1, true);
        // 2. Create the player
        player = ExoPlayerFactory.newSimpleInstance(new DefaultRenderersFactory(appContext), trackSelector, new DefaultLoadControl());
        // Bind the player to the view.
